...
void dfs(vector<vector<int>>& mat, int i, int j, int num, vector<int>& nums) {
    ...
    for (int k = 0; k < 8; ++k) {
        dfs(mat, i + dx[k], j + dy[k], num, nums);
    }
}
...
int mostFrequentPrime(vector<vector<int>>& mat) {
    ...
    for (auto& [num, frequency] : freq) {
        if (frequency > maxFreq || (frequency == maxFreq && num > ans)) {
            maxFreq = frequency;
            ans = num;
        }
    }
...
int main() {
    ...
    cout << mostFrequentPrime(mat) << endl; // output: 19
    vector<vector<int>> mat2 = {{7}};
    cout << mostFrequentPrime(mat2) << endl; // output: -1
    vector<vector<int>> mat3 = {{9, 7, 8}, {4, 6, 5}, {2, 8, 6}};
    cout << mostFrequentPrime(mat3) << endl; // output: 97
    return 0;
}
